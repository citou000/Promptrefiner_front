  ...
  <div class="px-4 w-[90%] sm:w-[50%]">
    ...
    <div class="mt-6">
      ...
      <div class="group bg-gray-100 p-3 rounded-md flex items-center justify-between">
        <pre class="w-full h-full whitespace-pre-wrap break-words text-sm">{{
          generatedPrompt
        }}</pre>
        <span
    ...
    <button
      class="outline-none w-full py-2 my-3 rounded-md text-white font-bold transition-colors"
      @click="generatePrompt"
      :disabled="!objective || !output || !context || loading"
      :class="[
